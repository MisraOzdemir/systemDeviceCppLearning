 ...
 *  1. It does not use an array to store all TIDs; instead, it uses pointers to reach the parent structure.
 *     TID is displayed in the right order using a recursive function.
 *  3. It does protect the output phase by including it in a critical section.
 */
...
void *run_thread(void *thread) {
  ...
  if (info->n > 0) {
    ...
    printf("T=%lld --> ", (unsigned long long) info->tid);
